...
export function Heading({ block }: { block: HeadingBlock }) {
  ...
  if (block.level === 1)
    return <h1 className={`${cls} text-3xl sm:text-4xl lg:text-5xl`}>{block.text}</h1>;
  if (block.level === 2)
    return <h2 className={`${cls} text-2xl sm:text-3xl`}>{block.text}</h2>;
  return <h3 className={`${cls} text-xl sm:text-2xl`}>{block.text}</h3>;
}

export function Text({ block }: { block: TextBlock }) {
  return (
    <p
      className={`leading-relaxed text-foreground/90 text-base sm:text-lg ${
...
export function BulletList({ block }: { block: BulletListBlock }) {
  ...
  return (
    <Tag
      className={`space-y-2 text-base sm:text-lg text-foreground/90 ${
...
export function Quote({ block }: { block: QuoteBlock }) {
  return (
    <blockquote className="border-l-4 border-primary bg-surface/60 px-4 py-3 italic text-foreground/90">
      <p className="text-base sm:text-lg">{block.text}</p>
...
export function Media({ block }: { block: MediaBlock }) {
  if (block.kind === "video") {
    return <video src={block.src} controls className="max-h-[60vh] w-full rounded-lg" />;
  }
